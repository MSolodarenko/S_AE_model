            ...
            elseif rw_iters > 1#19
                if total_len >= old_total_len
                    ...
                    try
                        ...
                        if _new_R <= r_min
                            _new_R = 0.9*r_min + (1.0-0.9)*best_R
                        elseif _new_R >= r_max
                            _new_R = 0.9*r_max + (1.0-0.9)*best_R
                        end
                        _new_W     = old_W - old_len_w*(old_W-old_old_W) /(old_len_w-old_old_len_w)
                        if _new_W <= w_min
                            _new_W = 0.9*w_min + (1.0-0.9)*best_W
                        elseif _new_W >= w_max
                            _new_W = 0.9*w_max + (1.0-0.9)*best_W
                        end
                        ...
                        if new_R1 <= r_min
                            new_R1 = 0.9*r_min + (1.0-0.9)*best_R
                            new_R1 = max(r_min, new_R1)
                        elseif new_R >= r_max
                            new_R1 = 0.9*r_max + (1.0-0.9)*best_R
                        ...
                        if new_W1 <= w_min
                            new_W1 = 0.9*w_min + (1.0-0.9)*best_W
                            new_W1 = max(w_min, new_W1)
                        elseif new_W1 >= w_max
                            new_W1 = 0.9*w_max + (1.0-0.9)*best_W
                    ...
                    elseif total_len == minimum([total_len, total_len1])
                        try
                            ...
                            if best_new_R <= r_min
                                best_new_R = 0.9*r_min + (1.0-0.9)*best_R
                            elseif best_new_R >= r_max
                                best_new_R = 0.9*r_max + (1.0-0.9)*best_R
                            end
                            best_new_W = old_W - old_len_w*(old_W-best_W)/(old_len_w-best_len_w)
                            if best_new_W <= w_min
                                best_new_W = 0.9*w_min + (1.0-0.9)*best_W
                            elseif best_new_W >= w_max
                                best_new_W = 0.9*w_max + (1.0-0.9)*best_W
                            end
                            ...
                            if new_R2 <= r_min
                                new_R2 = 0.9*r_min + (1.0-0.9)*best_R
                                new_R2 = max(r_min, new_R2)
                            elseif new_R2 >= r_max
                                new_R2 = 0.9*r_max + (1.0-0.9)*best_R
                            ...
                            if new_W2 <= w_min
                                new_W2 = 0.9*w_min + (1.0-0.9)*best_W
                                new_W2 = max(w_min, new_W2)
                            elseif new_W2 >= w_max
                                new_W2 = 0.9*w_max + (1.0-0.9)*best_W
