...
function utility(consumption, crra)
    if consumption > 0.0
        ...
        elseif crra >= 0.0
            return (consumption^(1-crra)-1)/(1-crra)
        else
            throw("crra is less than zero")
        end
    else
        return -10000000#-Inf
    end
...
function new_val_and_a1(value,aprime_nodes, income, value_tran_rhs,expectation_value_death_rhs, u_i,alpha_m_i,alpha_w_i ,a_min,a_max,a_nodes, aprime_len,val_len, number_a_nodes, beta, p_alpha, P_u, number_u_nodes, crra)

    new_value = Array{Float64}(undef,number_a_nodes)
    new_aprime_nodes = Array{Float64}(undef,number_a_nodes)

    function find_optimal_i(a_i,aprime_min,aprime_max)

        function f(aprime)
            consumption = income[a_i] - aprime

            if consumption > 0.0
                expectation = 0.0
                for u_prime_i in 1:number_u_nodes
                    expectation += evaluate(value_tran_rhs[u_prime_i], aprime)*P_u[u_i,u_prime_i]
                end
                res = utility(consumption, crra) + beta*(1-p_alpha)*expectation
                res += evaluate(expectation_value_death_rhs, aprime)
            else
                res = -100000000
            end
            return -res
        ...
        if aprime_min <= new_aprime_nodes[a_i] && new_aprime_nodes[a_i] <= aprime_max
            new_value[a_i] = -Optim.minimum(res)
        else
            new_aprime_nodes[a_i] = min(max(aprime_min,new_aprime_nodes[a_i]),aprime_max)
            new_value[a_i] = -f(new_aprime_nodes[a_i])
        end

        #a_candidates = collect(range(aprime_min; stop=aprime_max, length=5))
        #display(plot(a_candidates,f.(a_candidates)))

    end

    # simple monotonicity
    for i in 1:number_a_nodes
        if i > 1
            t_aprime_min = new_aprime_nodes[i-1]
            #t_aprime_max = income[i]
            t_aprime_max = income[i] + new_aprime_nodes[i-1] - income[i-1]
        else
            t_aprime_min = a_nodes[1]
            t_aprime_max = income[i]
        end

        find_optimal_i(i,t_aprime_min,t_aprime_max)

    end

    return new_value, new_aprime_nodes
end
...
function find_policy(a_min,a_max,a_nodes,r,w, income,earnings, val_tol, number_a_nodes, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, beta, p_alpha, P_zeta, P_u, stat_P_u, P_alpha, number_u_nodes, crra)
    ...
    while val_len > val_tol && val_iters < val_maxiters
            ...
            for alpha_m_i in 1:number_alpha_m_nodes
                for alpha_w_i in 1:number_alpha_w_nodes
                    for zeta_prime_i in 1:number_zeta_nodes
                        value_tran[:,u_i,alpha_m_i,alpha_w_i] .+= P_zeta[zeta_prime_i].*value[:,u_i,zeta_prime_i,alpha_m_i,alpha_w_i]
                    end
                end
            end
        end
        ...
        b_lowerbar = (beta/(1-beta))*minimum(new_value - value)
        b_upperbar = (beta/(1-beta))*maximum(new_value - value)

        val_len = maximum(abs,new_value-value)/maximum(abs,new_value)

        val_sumlen = sum(abs,new_value-value)/maximum(abs,new_value)
        ...
        if old_val_len > val_len && stable
            ...
            aprime_nodes .= new_aprime_nodes
        else
            stable = false
            value .= new_value.*val_Delta .+ value.*(1-val_Delta)
            aprime_nodes .= new_aprime_nodes.*val_Delta .+ aprime_nodes.*(1-val_Delta)
        end
...
function find_stationary_distribution_pdf(a1_nodes,a_min,a_max,a_nodes, distr_tol, number_a_nodes, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, p_alpha, P_zeta, P_u, stat_P_u, P_alpha, number_u_nodes)
    ...
    policy = zeros(number_asset_grid,number_u_nodes,number_zeta_nodes,number_alpha_m_nodes,number_alpha_w_nodes)
    ...
    Threads.@threads for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
        policy_function = Schumaker(a_nodes,a1_nodes[:,u_i,zeta_i,alpha_m_i,alpha_w_i]; extrapolation = (Linear,Linear))
        for a_i in 1:number_asset_grid
            a1 = evaluate(policy_function, asset_grid[a_i])
            policy[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = a1
            j_1 = sum(a1 .>= asset_grid)
            j = j_1 + 1

            a1_indices[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = max(1,min(Int64(round(j_1;digits=0)),number_asset_grid))
    ...
    while distr_len > distr_tol && distr_iters < distr_maxiters
        ...
        distr_a1_z0 .= 0.0
        Threads.@threads for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
            non_zero_asset_grid_iters = findall(!iszero,distr[:,u_i,zeta_i,alpha_m_i,alpha_w_i])
            for a_i in non_zero_asset_grid_iters#1:number_asset_grid#
                j_1 = a1_indices[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
                distr_a1_z0[j_1,u_i,zeta_i,alpha_m_i,alpha_w_i] += distr[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]*lottery_prob_1[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
                if j_1 != number_asset_grid
                    distr_a1_z0[j_1+1,u_i,zeta_i,alpha_m_i,alpha_w_i] += distr[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]*lottery_prob_2[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
                end
            end
        end
        ...
        distr_len = maximum(abs,distr-new_distr)
        distr .= new_distr
        distr_iters += 1
...
function find_aggregate_capital_labour_demand_supply(number_asset_grid,asset_grid,policy,density_distr,r,w, labour_excess, labour_d, labour_s, capital_excess, capital_d)
    # find capital demand and supply
    K_demand = sum(density_distr.*capital_d)
    K_supply = sum(density_distr.*policy)
    # find excess capital demand on capital market
    Capital_excess = sum(density_distr.*capital_excess)/sum(density_distr.*(capital_d.+policy))

    # find labour demand and supply
    L_demand = sum(density_distr.*labour_d)
    L_supply = sum(density_distr.*labour_s)
...
function quick_calculate_results(number_asset_grid,asset_grid,policy, a1_indices, lottery_prob_1, lottery_prob_2,density_distr,r,w, occ_choice, income, earnings, capital_d, credit, output, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, p_alpha, P_zeta, P_u, stat_P_u, P_alpha, number_u_nodes, cost_of_employing, z_m_nodes, z_w_nodes, lambda,delta,gamma,eta,theta,c_e, labour_d)

    number_non_zero_asset_grid = 0
    for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
        temp = findlast(round.(density_distr[:,u_i,zeta_i,alpha_m_i,alpha_w_i]; digits=7) .!= 0.0)
        if isnothing(temp)
            temp = 0
        end
    ...
    agg_capital = sum(density_distr.*capital_d)
    agg_output = sum(density_distr.*output)
    capital_to_output = agg_capital/agg_output

    agg_credit = sum(density_distr.*credit)
    credit_to_output = agg_credit/agg_output

    w_choice = copy(occ_choice)
    ent_choice = copy(occ_choice)
    emp_choice = copy(occ_choice)
    se_choice = copy(occ_choice)
    Threads.@threads for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
        for a_i in 1:number_asset_grid
            ...
            if ent_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] == 1
                ent_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = 0
            else
                ent_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = 1
            end
            ...
            if se_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] != 2
                se_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = 0
            else
                se_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i] = 1
            end
        end
    end

    share_of_workers = sum(density_distr.*w_choice)
    share_of_self_employed = sum(density_distr.*se_choice)
    share_of_employers = sum(density_distr.*emp_choice)

    consumption = income.-policy
    avg_log_c = sum(density_distr.*log.(max.(1e-12,consumption)))
    var_log_c = sum(density_distr.*(log.(max.(1e-12,consumption)).- avg_log_c).^2)
    ...
    =#
    avg_log_income = sum(density_distr.*log.(max.(1e-12,earnings)))
    var_log_income = sum(density_distr.*(log.(max.(1e-12,earnings)).- avg_log_income).^2)

    density_distr_w = density_distr.*w_choice
    density_distr_se = density_distr.*se_choice
    ...
    income_workers = income.*w_choice
    ...
    income_entrepreneurs = income.*ent_choice
    ...
    agg_labour = sum(density_distr.*labour_d)
    ...
    agg_ent_earnings = sum(density_distr.*earnings.*(1.0.-w_choice))
    ...
    if calc_add_results
        ...
        density_distr_pr = copy(density_distr).*0.0
        density_distr_w_pr = copy(density_distr_w).*0.0
        density_distr_se_pr = copy(density_distr_se).*0.0
        ...
        Threads.@threads for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
            for a_i in 1:number_asset_grid
                ...
                density_distr_se_pr[j_1,u_i,zeta_i,alpha_m_i,alpha_w_i] += density_distr_se[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]*lottery_prob_1[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
                ...
                if j_1 != number_asset_grid
                    ...
                    density_distr_se_pr[j_1+1,u_i,zeta_i,alpha_m_i,alpha_w_i] += density_distr_se[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]*lottery_prob_2[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
        ...
        occ_trans_matrix[1,1] = sum(w_choice.*density_distr_w_pr)/max(1e-6,sum(density_distr_w))
        occ_trans_matrix[1,2] = sum(se_choice.*density_distr_w_pr)/max(1e-6,sum(density_distr_w))
        occ_trans_matrix[1,3] = sum(emp_choice.*density_distr_w_pr)/max(1e-6,sum(density_distr_w))
        occ_trans_matrix[2,1] = sum(w_choice.*density_distr_se_pr)/max(1e-6,sum(density_distr_se))
        occ_trans_matrix[2,2] = sum(se_choice.*density_distr_se_pr)/max(1e-6,sum(density_distr_se))
        occ_trans_matrix[2,3] = sum(emp_choice.*density_distr_se_pr)/max(1e-6,sum(density_distr_se))
        occ_trans_matrix[3,1] = sum(w_choice.*density_distr_emp_pr)/max(1e-6,sum(density_distr_emp))
        occ_trans_matrix[3,2] = sum(se_choice.*density_distr_emp_pr)/max(1e-6,sum(density_distr_emp))
        occ_trans_matrix[3,3] = sum(emp_choice.*density_distr_emp_pr)/max(1e-6,sum(density_distr_emp))
...
function draw_occupational_choice(z_m_nodes,z_w_nodes,occ_choice, a_i, asset_grid,r,w,lambda,delta,gamma,eta,theta,c_e)
    z_m_nodes_vector = sort(vec(z_m_nodes))
    z_m_nodes_vector = collect(range(z_m_nodes_vector[1]; stop=z_m_nodes_vector[end], length = 100))
    z_w_nodes_vector = sort(vec(z_w_nodes))
    z_w_nodes_vector = collect(range(z_w_nodes_vector[1]; stop=z_w_nodes_vector[end], length = 100))

    occ_choice_table = zeros(length(z_m_nodes_vector), length(z_w_nodes_vector))
    #=for (u_i,(zeta_i,(alpha_m_i,alpha_w_i))) in collect(Iterators.product(1:number_u_nodes,Iterators.product(1:number_zeta_nodes,Iterators.product(1:number_alpha_m_nodes,1:number_alpha_w_nodes))))
    z_m = z_m_nodes[u_i,alpha_m_i,zeta_i]
    z_w = z_w_nodes[u_i,alpha_m_i,alpha_w_i]

    z_m_i = findfirst(z_m_nodes_vector .== z_m)
    z_w_i = findfirst(z_w_nodes_vector .== z_w)

    occ_choice_table[z_m_i,z_w_i] = occ_choice[a_i,u_i,zeta_i,alpha_m_i,alpha_w_i]
    end=#
    for (z_m_i,z_w_i) in collect(Iterators.product(1:length(z_m_nodes_vector),1:length(z_w_nodes_vector)))
        occ_choice_table[z_m_i,z_w_i] = compute_income_profile(asset_grid[a_i],z_m_nodes_vector[z_m_i],z_w_nodes_vector[z_w_i],r,w, lambda, delta, gamma, eta, theta, c_e)[1]
    end

    display(heatmap(
                    z_m_nodes_vector,z_w_nodes_vector,
                    #1:length(z_m_nodes_vector),1:length(z_w_nodes_vector),
                    occ_choice_table,
                    c=cgrad([:white, :grey, :black]),
                    #c=cgrad([:red, :yellow, :green]),
                    xlabel="z_m", ylabel="z_w",
                    title="Occupational choice function at $(round(asset_grid[a_i];digits=2))"))
    path = "/Users/maxsolodarenko/OneDrive - University of Glasgow/PhD/Year 2/AllubErosa Transitional dynamics/Code/Test_firm_r_w/Results/Figures/"
    if Sys.iswindows()
        path = "C:\\Users\\2288144s\\OneDrive - University of Glasgow\\PhD\\Year 2\\AllubErosa Transitional dynamics\\Code\\Test_firm_r_w\\Results\\Figures\\"
    end
    savefig("$(path)occ_choice_function_asset_level_$(round(asset_grid[a_i];digits=2)).png")

end

function AllubErosa(r,w, gps, gaps, ps, ao)
    distr_tol = gps[3]
    val_tol = gps[4]
    ...
    income, earnings = compute_income_and_earnings(a_nodes,number_a_nodes,r, w, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, lambda, delta, gamma, eta, theta, c_e, z_m_nodes, z_w_nodes, number_u_nodes)

    # VFI that finds policy function for future capital
    a1_nodes, value = find_policy(a_min,a_max,a_nodes,r,w, income,earnings, val_tol, number_a_nodes, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, beta, p_alpha, P_zeta, P_u, stat_P_u, P_alpha, number_u_nodes, crra)

    ##############Compute K_supply

    ########## Compute the stationary distribution of assets
    density_distr, number_asset_grid, asset_grid, policy, a1_indices, lottery_prob_1, lottery_prob_2 = find_stationary_distribution_pdf(a1_nodes,a_min,a_max,a_nodes, distr_tol, number_a_nodes, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, p_alpha, P_zeta, P_u, stat_P_u, P_alpha, number_u_nodes)
    # calculate income profiles for different households
    occ_choice, income, earnings, capital_excess, capital_d, credit, labour_excess, labour_d, labour_s, deposit, output, cost_of_employing, managerial_input = compute_income_profile(asset_grid,number_asset_grid,r,w, number_zeta_nodes, number_alpha_m_nodes, number_alpha_w_nodes, lambda, delta, gamma, eta, theta, c_e, z_m_nodes, z_w_nodes, number_u_nodes)

    K_demand, K_supply, L_demand, L_supply, Capital_excess, Labor_excess = find_aggregate_capital_labour_demand_supply(number_asset_grid,asset_grid,policy,density_distr,r,w, labour_excess, labour_d, labour_s, capital_excess, capital_d)
